        ...
        mod rules {
            ...
            pub mod visible {
                ...
                pub fn grammar_rules(
                    ...
                    state.sequence(|state| {
                        self::SOI(state)
                            ...
                            .and_then(|state| {
                                state.sequence(|state| {
                                    self::grammar_rule(state)
                                        .and_then(|state| super::hidden::skip(state))
                                        .and_then(|state| {
                                            state.sequence(|state| {
                                                state.optional(|state| {
                                                    self::grammar_rule(state).and_then(|state| {
                                                        state.repeat(|state| {
                                                            state.sequence(|state| {
                ...
                pub fn grammar_rule(
                    ...
                    state.rule(Rule::grammar_rule, |state| {
                        state.sequence(|state| {
                            self::identifier(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::assignment_operator(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| state.optional(|state| self::modifier(state)))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::opening_brace(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::expression(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::closing_brace(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn modifier(
                    ...
                    self::silent_modifier(state)
                        .or_else(|state| self::atomic_modifier(state))
                        .or_else(|state| self::compound_atomic_modifier(state))
                        .or_else(|state| self::non_atomic_modifier(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn silent_modifier(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                ...
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn compound_atomic_modifier(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::compound_atomic_modifier, |state| {
                        state.match_string("$")
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn non_atomic_modifier(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                ...
                pub fn expression(
                    ...
                    state.rule(Rule::expression, |state| {
                        state.sequence(|state| {
                            self::term(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| {
                    ...
                    state.rule(Rule::term, |state| {
                        state.sequence(|state| {
                            state
                                .sequence(|state| {
                                    state.optional(|state| {
                                        self::prefix_operator(state).and_then(|state| {
                                            state.repeat(|state| {
                                                state.sequence(|state| {
                                                    super::hidden::skip(state).and_then(|state| {
                                                        self::prefix_operator(state)
                                                    })
                                                })
                                            })
                                        })
                                ...
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::node(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| {
                                    state.sequence(|state| {
                                        state.optional(|state| {
                                            self::postfix_operator(state).and_then(|state| {
                                                state.repeat(|state| {
                                                    state.sequence(|state| {
                                                        super::hidden::skip(state).and_then(
                                                            |state| self::postfix_operator(state),
                                                        )
                                                    })
                                                })
                                            })
                ...
                pub fn node(
                    ...
                    state
                        .sequence(|state| {
                            self::opening_paren(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::expression(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::closing_paren(state))
                        })
                        .or_else(|state| self::terminal(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn terminal(
                    ...
                    self::_push(state)
                        .or_else(|state| self::peek_slice(state))
                        .or_else(|state| self::identifier(state))
                        .or_else(|state| self::string(state))
                        .or_else(|state| self::insensitive_string(state))
                        .or_else(|state| self::range(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn prefix_operator(
                    ...
                    self::positive_predicate_operator(state)
                        .or_else(|state| self::negative_predicate_operator(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn postfix_operator(
                    ...
                    self::optional_operator(state)
                        .or_else(|state| self::repeat_operator(state))
                        .or_else(|state| self::repeat_once_operator(state))
                        .or_else(|state| self::repeat_exact(state))
                        .or_else(|state| self::repeat_min(state))
                        .or_else(|state| self::repeat_max(state))
                        .or_else(|state| self::repeat_min_max(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn positive_predicate_operator(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::positive_predicate_operator, |state| {
                        state.match_string("&")
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn negative_predicate_operator(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::negative_predicate_operator, |state| {
                        state.match_string("!")
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn sequence_operator(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                ...
                pub fn repeat_exact(
                    ...
                    state.rule(Rule::repeat_exact, |state| {
                        state.sequence(|state| {
                            self::opening_brace(state)
                                .and_then(|state| super::hidden::skip(state))
                ...
                pub fn repeat_min(
                    ...
                    state.rule(Rule::repeat_min, |state| {
                        state.sequence(|state| {
                            self::opening_brace(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::number(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::comma(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::closing_brace(state))
                        })
                    })
                ...
                pub fn repeat_max(
                    ...
                    state.rule(Rule::repeat_max, |state| {
                        state.sequence(|state| {
                            self::opening_brace(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::comma(state))
                ...
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn repeat_min_max(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::repeat_min_max, |state| {
                        state.sequence(|state| {
                            self::opening_brace(state)
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::number(state))
                ...
                pub fn integer(
                    ...
                    state.rule(Rule::integer, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            self::number(state).or_else(|state| {
                                state.sequence(|state| {
                                    state
                                        .match_string("-")
                                        .and_then(|state| {
                                            state.repeat(|state| state.match_string("0"))
                                        })
                                        .and_then(|state| state.match_range('1'..'9'))
                                        .and_then(|state| {
                                            state.optional(|state| self::number(state))
                                        })
                                })
                            })
                        })
                ...
                pub fn _push(
                    ...
                    state.rule(Rule::_push, |state| {
                        state.sequence(|state| {
                            state
                                .match_string("PUSH")
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::opening_paren(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::expression(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::closing_paren(state))
                ...
                pub fn peek_slice(
                    ...
                    state.rule(Rule::peek_slice, |state| {
                        state.sequence(|state| {
                            state
                                .match_string("PEEK")
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::opening_brack(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| state.optional(|state| self::integer(state)))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::range_operator(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| state.optional(|state| self::integer(state)))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::closing_brack(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn identifier(
                    ...
                    state.rule(Rule::identifier, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                state
                                    .lookahead(false, |state| state.match_string("PUSH"))
                                    .and_then(|state| {
                                        state.match_string("_").or_else(|state| self::alpha(state))
                                    })
                                    .and_then(|state| {
                                        state.repeat(|state| {
                                            state
                                                .match_string("_")
                                                .or_else(|state| self::alpha_num(state))
                                        })
                                    })
                            })
                        })
                ...
                pub fn string(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::string, |state| {
                            state.sequence(|state| {
                                self::quote(state)
                                    .and_then(|state| self::inner_str(state))
                                    .and_then(|state| self::quote(state))
                            })
                        })
                    })
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn insensitive_string(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::insensitive_string, |state| {
                        state.sequence(|state| {
                            state
                                .match_string("^")
                                .and_then(|state| super::hidden::skip(state))
                    ...
                    state.rule(Rule::range, |state| {
                        state.sequence(|state| {
                            self::character(state)
                                ...
                                .and_then(|state| self::range_operator(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::character(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn character(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::character, |state| {
                            state.sequence(|state| {
                                self::single_quote(state)
                                    .and_then(|state| self::inner_chr(state))
                                    .and_then(|state| self::single_quote(state))
                            })
                        })
                    })
                }
                ...
                pub fn inner_str(
                    ...
                    state.rule(Rule::inner_str, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                let strings = ["\"", "\\"];
                                state.skip_until(&strings).and_then(|state| {
                                    state.optional(|state| {
                                        state.sequence(|state| {
                                            self::escape(state)
                                                .and_then(|state| self::inner_str(state))
                ...
                pub fn inner_chr(
                    ...
                    state.rule(Rule::inner_chr, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            self::escape(state).or_else(|state| self::ANY(state))
                        })
                    })
                }
                #[inline]
                            ...
                            state.sequence(|state| {
                                state.match_string("\\").and_then(|state| {
                                    state
                                        ...
                                        .or_else(|state| state.match_string("0"))
                                        .or_else(|state| state.match_string("'"))
                                        .or_else(|state| self::code(state))
                                        .or_else(|state| self::unicode(state))
                                })
                            })
                        })
                    })
                ...
                pub fn code(
                    ...
                    state.rule(Rule::code, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                state
                                    .match_string("x")
                                    .and_then(|state| self::hex_digit(state))
                                    .and_then(|state| self::hex_digit(state))
                            })
                        })
                    })
                        ...
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                state
                                    ...
                                    .and_then(|state| {
                                        state.sequence(|state| {
                                            self::hex_digit(state)
                                                .and_then(|state| self::hex_digit(state))
                                                .and_then(|state| {
                                                    state.optional(|state| self::hex_digit(state))
                                                })
                                                .and_then(|state| {
                                                    state.optional(|state| self::hex_digit(state))
                                                })
                ...
                pub fn hex_digit(
                    ...
                    state.rule(Rule::hex_digit, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state
                                .match_range('0'..'9')
